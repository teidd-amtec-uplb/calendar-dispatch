...
export default function DocumentExportModal({ dispatchId, dispatchNumber, token, onClose }: Props) {
  ...
  const [assignedPersonnel, setAssignedPersonnel] = useState<{name:string;email:string|null;role:string}[]>([]);
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      setAssignedPersonnel(json.assignedPersonnel ?? []);
      setStatus("done");
    } catch {
            ...
            <>
              ...
              {emailError ? (
                <div className="flex items-start gap-2 px-3 py-2 rounded-lg bg-amber-50 border border-amber-200">
                  ...
                  <div>
                    <p className="text-xs font-semibold text-amber-800">Email could not be sent</p>
                    <p className="text-xs text-amber-700 mt-0.5">{emailError}</p>
                    <p className="text-xs text-amber-600 mt-1">
                      Please download the documents below and share them with your assigned personnel manually.
                    </p>
                  </div>
                </div>
              ) : emailSent ? (
                <div className="space-y-1.5">
                  ...
                  {assignedPersonnel.length > 0 && (
                    <div className="px-3 py-2 rounded-lg bg-blue-50 border border-blue-100">
                      <p className="text-xs font-semibold text-blue-700 mb-1">📋 Please forward to assigned personnel:</p>
                      <div className="space-y-0.5">
                        {assignedPersonnel.map((p, i) => (
                          <p key={i} className="text-xs text-blue-600">
                            <span className="font-medium">{p.name}</span>
                            {p.email && <span className="text-blue-400 ml-1">— {p.email}</span>}
                            <span className="text-blue-300 ml-1">({p.role.replace(/_/g,' ')})</span>
                          </p>
                        ))}
                      </div>
                    </div>
